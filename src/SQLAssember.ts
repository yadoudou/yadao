...
class SQLAssember {
    ...
    getSelect ( tables, fields, conds = null, options = null, appends = null ) {
        var sql = 'SELECT ';
        // 1.options
        if ( options ) {
    ...
    getUpdate ( table, row, conds, options, appends ) {
    ...
    getDelete ( table, conds = null, options = null, appends = null ) {
        return this._makeUpdateOrDelete( table, null, conds, options, appends );
    }

    getInsert ( table, row, options = null, onDup = null ) {
        var sql = 'INSERT ';
        if ( options ) {
            options = this._makeList( options, LIST_TYPE.COM, ' ' );
            sql += `${options} `;
    ...
    _makeUpdateOrDelete ( table, row, conds, options, appends ) {
        // 1. options
        var sql = '';
        if ( options ) {
    ...
    _makeList ( list, type: LIST_TYPE = LIST_TYPE.SET, cut: string = ', ' ) {
    ...
    _makeConds ( conds ) {
        ...
        Object.entries(conds).map( ( [ key, value ]) => {
            ...
            } else if ( this._isInt(key) && ( typeof value === 'string')) {
